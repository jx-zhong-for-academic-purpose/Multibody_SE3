...
class MultiTaskDetectionLoss(torch.nn.Module):
    ...
    def forward(self, wts, label, y, gt_R, gt_T=None):
        ...
        if na == 1:
            ...
            #'''
            preds = label.cuda()
            confidence = torch.take_along_dim(wts, preds.unsqueeze(1), dim=1).squeeze(1)
            # '''
...
class TripletBatchLoss(nn.Module):
    ...
    def forward(self, src, tgt, T, equi_src=None, equi_tgt=None):
        ...
        self.gt_idx = torch.arange(src.shape[0], dtype=torch.int32).unsqueeze(1).expand(-1,
                                                                                        self.k_precision).contiguous().int().to(
            self.device)
