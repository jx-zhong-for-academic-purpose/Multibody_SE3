...
def preprocess_input(x, na, add_center=True, extra_feats=None):
    ...
    xyz = x[:,:,:3]
    feats = sptk.get_occupancy_features(x, na, add_center)
    return zptk.SphericalPointCloud(xyz.permute(0,2,1).contiguous(),
                                    feats if extra_feats is None else torch.cat([feats, extra_feats], -1),
                                    None)
...
class BasicSO3ConvBlock(nn.Module):
    ...
    def get_anchor(self):
        return torch.from_numpy(sptk.get_anchors(self.params[0]['args']['kanchor']))
...
class RelSO3OutBlockR(nn.Module):
    ...
    def forward(self, f1, f2, x1, x2):
        ...
        #print('f1 before pooling', f1.shape)
        f1 = self._pooling(sp1)
        f2 = self._pooling(sp2)

        nb = f1.shape[0]
        na = f1.shape[2]
        #print('f1 after pooling', f1.shape)
        ...
        '''
        f1 before pooling torch.Size([3, 256, 64, 60])
        f1 after pooling torch.Size([3, 256, 60])
        x_out:  torch.Size([3, 512, 60, 60])
        '''
        ...
        #print(confidence.shape, y.shape) #torch.Size([3, 60, 60]) torch.Size([3, 4, 60, 60])
